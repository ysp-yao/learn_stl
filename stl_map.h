...
#endif
  self& operator++() { increamnet(); return *this; }
...
protected:
  ...
  void put_node(link_type p) { rb_tree_node_allocator::dealoocate(p); }
...
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::insert_unique(const Value& x) {
  ...
  if (comp)
    if (j == being()) return pair<iterator, bool>(__insert(x, y, v), true);
    else --j;
